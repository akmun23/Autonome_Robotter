...
GUI::GUI(const string& winName):graveyardRect(600, 75, 50, 50){
    canvas = Mat3b(800, 800, Vec3b(255,255,255)); //Creates drawing surface, canvas.
    boards = startUp();
}

void GUI::clearVecInt(vector<int>& vector){
...
void GUI::clearVecString(vector<string>& vector){
    vector.resize(0);
}
...
void GUI::updateText(){
    ...
    ((playerTurn%2 == 0) ? playerTurnString = spiller : playerTurnString = robot);
...
int GUI::findInVector(string input[], string wantFound){
    for(int i = 0; i < 8; i++){
...
void GUI::posToStringConvert(int x, int y){
...
void GUI::Draw(){
    ...
    if(startUpMain){
        ...
        resize(imread("/home/aksel/Documents/GitHub/Autonome_Robotter/GUI/GUI VERSIONS/V2.0/graveyard.jpg"), graveyardDownSized, Size(graveyardRect.width, graveyardRect.height), INTER_LINEAR);
...
bool GUI::doesRectContainChecker(int x, int y, vector<Circle> enemyCheckerVec){
    for(int i = 0; i < 12; i++){
...
bool GUI::doesPosContainRect(int x, int y){
    for(int i = 0; i < 64; i++){
...
bool GUI::isJumpPossible(int x, int y, vector<Circle> enemyCheckerVec){
...
bool GUI::isGameWon(vector<Circle> enemyCheckerVec, int playerTurn){
    for(int i = 0; i < 64; i++){
        for(int j = 0; j < 12; j++){
            if(rectangles[i].contains(Point(enemyCheckerVec[j].x, enemyCheckerVec[j].y)) && movesPossible.size() > 0){
                return false;
            }
        }
        if(i == 64){
...
void GUI::setJumpPerformed(bool& jumpPerformed, bool TrueFalse){
    jumpPerformed = TrueFalse;
    cout << "JumpPerformed was updated: " << jumpPerformed << endl;
}

void GUI::mouseCallBack(int event, int userX, int userY, int flags, void* userdata) {
    GUI* gui = static_cast<GUI*>(userdata);
    if (gui != nullptr) {
        gui->callBackFunc(event, userX, userY, flags, userdata);
    }
}

//callBack function, that runs every time you click.
void GUI::callBackFunc(int event, int userX, int userY, int flags, void* userdata){
    jumpsPossible = jumpPossible(playerTurn, boards);
    moreMove = moreMoveCheck(jumpsPossible, stringPos[1]);
    movesPossible = movePossible(playerTurn, boards, jumpsPossible, moreMove, stringPos[1]);
    if(selected.size() != 0 && event == EVENT_LBUTTONDOWN){ //Runs this code left mouse button clicked and "Rect" is already selected.
            ...
            if(rectangles[i].contains(Point(userX,userY))){ //Checks if the click is inside of the game board.
                ...
                if(rectangles[selected[2]].x == rectangles[i].x && rectangles[selected[2]].y == rectangles[i].y){
                    ...
                    if(selected[3] == 1){ //If there is a single jump possible run this code.
                        ...
                        posToStringConvert(rectangles[selected[4]].x, rectangles[selected[4]].y);
                    }
                        ...
                        if(rectangles[i].x > rectangles[selected[2]].x){ //If "Rect" that was clicked is to the right of the earlier selected "Rect".
                            posToStringConvert(rectangles[selected[4]].x, rectangles[selected[4]].y); //Gets string position for possible jump to the right.
                        }
                        else{
                            posToStringConvert(rectangles[selected[5]].x, rectangles[selected[5]].y); //Gets string position for possible jump to the left.
                        }
                    }
                    else{ //If no jumps possible.
                        posToStringConvert(rectangles[i].x, rectangles[i].y);
                    ...
                    if(move(playerTurn, boards, redCheckers, blackCheckers, stringPos[0], stringPos[1])){
                        ...
                        cout << "Is game won: " << isGameWon(bCheckers, playerTurn) << endl;
                    ...
                    if(selected[3] == 1){
                        ...
                        if(rectangles[i].x > rectangles[selected[2]].x){
                            ...
                            posToStringConvert(rectangles[selected[5]].x, rectangles[selected[5]].y);
                        }
                    }
                    else{
                    ...
                    if(move(playerTurn, boards, redCheckers, blackCheckers, stringPos[0], stringPos[1])){
                        ...
                        if(isJumpPossible(rectangles[i].x, rectangles[i].y, rCheckers) && jumpPerformed){
                            ...
                            posToStringConvert(rectangles[i].x, rectangles[i].y); //Problem her
        ...
        for(int i = 0; i < 64; i++){
            if(rectangles[i].contains(Point(userX,userY))){ //Checks if click is inside game board.
                ...
                for(int j = 0; j < 12; j++){
                    if(rCheckers[j].contains(Point(userX, userY)) && playerTurn == 2){ //Checks if there is a checker on the Rect, corresponding to the which players turn it is.
                        canvas(rectangles[i]) = Vec3b(0,0,255); //Highlights the chosen checker.
                        ...
                        posToStringConvert(rectangles[i].x, rectangles[i].y); //Converts position of selected checker to string notation and saves the value in "stringPos".
                    ...
                    else if(bCheckers[j].contains(Point(userX, userY)) && playerTurn == 1){ //Same as for the previous section.
